...
struct KairoCalendarView: View {
    ...
    private func dateCellView(_ date: Date) -> some View {
        ...
        return VStack(spacing: 2) {
            Text("\(dayNumber)")
                ...
                .foregroundColor(
                    isSelected ? .white :
                    isToday ? KairoColors.accentAdaptive :
        ...
        .background(
            RoundedRectangle(cornerRadius: 10)
                .fill(
                    isSelected ? KairoColors.accentAdaptive :
                    isToday ? KairoColors.accentAdaptive.opacity(0.1) :
                    Color.clear
                )
        )
    }
    ...
    @State private var didSetupDaily = UserDefaults.standard.bool(forKey: "kairo_daily_focus_setup")

    private var quickActions: some View {
        VStack(alignment: .leading, spacing: 14) {
            // Quick setup: Daily 30-min focus
            if !didSetupDaily {
                Button {
                    setupDailyFocus()
                } label: {
                    HStack(spacing: 12) {
                        ZStack {
                            Circle()
                                .fill(
                                    LinearGradient(
                                        colors: [Color(hex: 0x9B59B6), Color(hex: 0x3498DB)],
                        ...
                        VStack(alignment: .leading, spacing: 3) {
                            Text("Quick Setup: Daily Focus")
                                .font(.system(size: 15, weight: .bold))
                                .foregroundColor(KairoColors.primaryAdaptive)
                            Text("30 min every day with daily reminders at 9 AM")
                                .font(.system(size: 12))
                                .foregroundColor(KairoColors.mutedAdaptive)
                        }
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 14)
                            .fill(
                                LinearGradient(
                                    colors: [Color(hex: 0x9B59B6).opacity(0.08), Color(hex: 0x3498DB).opacity(0.08)],
                                    startPoint: .leading,
                                    endPoint: .trailing
                                )
                            )
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 14)
                            .stroke(Color(hex: 0x9B59B6).opacity(0.2), lineWidth: 1)
                    )
                }
            }
            ...
            if calendarManager.hasAccess {
                ...
                Button {
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 12)
                            .fill(KairoColors.accentAdaptive.opacity(0.08))
                    )
                }
            }
        }
    ...
    // MARK: - Daily Focus Setup

    private func setupDailyFocus() {
        // Create a "Daily Focus" goal: 30 min every day
        let goal = CustomGoal(
            title: "Daily Focus",
            targetValue: 30,
            goalType: .minutes,
        ...
        // Schedule on calendar if we have access (all 7 days)
        if calendarManager.hasAccess {
        ...
        // Mark as set up
        UserDefaults.standard.set(true, forKey: "kairo_daily_focus_setup")
        didSetupDaily = true
